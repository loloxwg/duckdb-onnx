...
struct OutletId {
  ...
  bool operator!=(const OutletId &other) const { return !(*this == other); }
...
// 从整数转换为 OutletId
inline OutletId MakeOutletId(size_t node) { return OutletId(node, 0); }
...
struct InletId {
  ...
  bool operator!=(const InletId &other) const { return !(*this == other); }
...
template <typename F> class Outlet {
public:
...
/// model.
template <typename F, typename O> class Node {
public:
  ...
  template <typename OpType> OpType *op_as_mut() {
